...
var CausalDiscovery = (function () {
    ...
    function parseCSV(text) {
        ...
        for (var c = 0; c < p; c++) {
            ...
            // 2. Require: ≥80% numeric, ≥2 distinct values, uniqueness ≤50%
            //    (the uniqueness check catches purely numeric free-text IDs)
            if (fracNumeric >= 0.8 && nDistinct >= 2 && fracDistinct <= 0.5) {
                numericCols.push(c);
            } else if (fracNumeric >= 0.8 && nDistinct >= 2 && fracDistinct > 0.5) {
                droppedCols.push(colName + ' (ID-like — too many unique values)');
            } else {
